...
class EMAIncrementalNet(BaseNet):
    ...
    def merge_fc(self, nb_classes):
        ...
        if self.fc is not None:
            ...
            fc.weight.data[:nb_output] = weight
            fc.bias.data[:nb_output] = bias
            weight = copy.deepcopy(self.new_fc.weight.data)
            bias = copy.deepcopy(self.new_fc.bias.data)
            fc.weight.data[nb_output:] = weight
            fc.bias.data[nb_output:] = bias
    ...
    # def create_new_fc(self, nb_classes):
    #     fc = self.generate_fc(self.feature_dim, nb_classes)
    #     self.new_fc = fc 
...
class AKAIncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        ...
        fc = self.generate_fc(self.feature_dim, nb_classes//4)
